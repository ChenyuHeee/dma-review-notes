...
Hamilton 提出过一个谜题：沿着正十二面体（dodecahedron）的棱行走，从某个城市（顶点）出发，访问其余 19 个城市各**恰好一次**，最后返回起点。
    ...
    \ / \   / \ / \ /
     O   P-Q---R   S
      \ /     \ /
       T       U
```
每个顶点代表一个城市，每条边代表十二面体的棱。问题等价于：该图中是否存在经过每个顶点恰好一次的**哈密顿回路**？

这个谜题直接引出了**哈密顿路径**和**哈密顿回路**的概念。
...
- 虽然所有顶点度数 $\ge 2$，但删除顶点集 $S = \{b, e\}$ 后，图分裂为 $\{a,c\}$、$\{d\}$、$\{f\}$ 三个分量，$3 > |S| = 2$，不满足必要条件
- 因此**不存在**哈密顿回路
...
```
- 每个顶点度数 $\ge 2$，结构紧凑
- 存在哈密顿回路如 $a \to b \to c \to f \to e \to d \to a$

**判断技巧**：
- 如果图中有度数为 1 的顶点 $\rightarrow$ 无哈密顿回路
- 如果图结构在删除某些顶点后分裂出太多分量 $\rightarrow$ 无哈密顿回路
- 如果所有顶点度数 $\ge n/2$（Dirac 定理）$\rightarrow$ 一定有哈密顿回路
